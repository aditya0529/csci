...
# Remote debugging support for PyCharm/IntelliJ
DEBUG_MODE = os.environ.get("PYCHARM_DEBUG", "false").lower() == "true"
DEBUG_HOST = os.environ.get("DEBUG_HOST", "localhost")
DEBUG_PORT = int(os.environ.get("DEBUG_PORT", "12345"))

if DEBUG_MODE:
    try:
        import pydevd_pycharm
        pydevd_pycharm.settrace(DEBUG_HOST, port=DEBUG_PORT, stdoutToServer=True, stderrToServer=True)
        print(f"🔧 Remote debugging enabled - connected to {DEBUG_HOST}:{DEBUG_PORT}")
    except ImportError:
        print("⚠️ pydevd_pycharm not available - remote debugging disabled")
...
class SecurityHubFinding:
    ...
    def __init__(self, payload):
        ...
        if self.product_name == "Security Hub" and 'Compliance' in payload and 'SecurityControlId' in payload[
            "Compliance"]:
            self.id = payload["Compliance"]["SecurityControlId"]
        elif self.product_name == "Inspector" and 'Vulnerabilities' in payload and 'Id' in payload["Vulnerabilities"][
            0]:
            self.id = payload["Vulnerabilities"][0]["Id"]
        elif self.product_name in ("IAM Access Analyzer", "Health", "GuardDuty") and 'Resources' in payload and 'Id' in \
                payload["Resources"]:
...
class Rule:
    ...
    """
        Returns:
            dict with keys:
              {"action": "suppress" | "severity_update", "new_severity": <str or None>}
            OR None if no match.
    """

    def matches(self, finding: SecurityHubFinding) -> Optional[Dict]:
        # 🔍 BREAKPOINT 13: Rule matching entry point
        logger.debug(f"🎯 Testing rule {self.id} against finding {finding.id}")
        
        # Step 1: ID matching
        id_match = False
        if finding.id == self.id:
            id_match = True
            logger.debug(f"✅ Exact ID match: {finding.id} == {self.id}")
        else:
            if self.product_name == "Inspector":
                # 🔍 BREAKPOINT 14: Inspector prefix matching
                id_prefix = re.compile(r'^' + self.id.strip("*"))
                logger.debug(f"🔍 Testing Inspector prefix pattern: ^{self.id.strip('*')}")
                if id_prefix.search(finding.id):
                    id_match = True
                    logger.debug(f"✅ Prefix ID match: {finding.id} matches ^{self.id.strip('*')}")
                else:
                    logger.debug(f"❌ No ID match: {finding.id} vs {self.id}")
        
        if not id_match:
            logger.debug(f"❌ Rule {self.id} failed ID match - skipping")
            return None
            
        # Step 2: Resource pattern matching
        # 🔍 BREAKPOINT 15: Resource pattern matching
        logger.debug(f"🔍 Testing resource pattern matching...")
        is_resource_id_type_match = self._match_resource_id_type(finding)
        
        if not is_resource_id_type_match:
            logger.debug(f"❌ Rule {self.id} failed resource pattern match - skipping")
            return None
            
        # Step 3: Account validation
        # 🔍 BREAKPOINT 16: Account inclusion/exclusion check
        account_match = False
        if self.account_inclusion:
            account_match = finding.finding_account_id in self.account_inclusion
            logger.debug(f"🔍 Account inclusion check: {finding.finding_account_id} in {self.account_inclusion} = {account_match}")
        else:
            account_match = finding.finding_account_id not in self.account_exception
            logger.debug(f"🔍 Account exclusion check: {finding.finding_account_id} not in {self.account_exception} = {account_match}")
            
        if not account_match:
            logger.debug(f"❌ Rule {self.id} failed account check - skipping")
            return None
            
        # Step 4: Extra resource pattern
        # 🔍 BREAKPOINT 17: Extra resource pattern check
        ...
        if self.extra_resource_pattern:
            ...
            logger.debug(f"🔍 Extra resource pattern: '{self.extra_resource_pattern}' vs '{finding.extra_resource_arn}' = {extra_pattern_match}")
        
        if not extra_pattern_match:
            logger.debug(f"❌ Rule {self.id} failed extra resource pattern - skipping")
            return None
            
        # 🔍 BREAKPOINT 18: Full rule match success
        logger.debug(f"🎉 FULL MATCH! Rule {self.id} matches finding {finding.id}")
        
        # Determine action
        if self.from_severity and self.to_severity and finding.label == self.from_severity:
            logger.debug(f"🔄 Severity update: {self.from_severity} → {self.to_severity}")
            return {"action": "severity_update", "new_severity": self.to_severity}
        else:
            logger.debug(f"🔒 Suppression action")
            return {"action": "suppress", "new_severity": None}

    def _match_resource_id_type(self, finding: SecurityHubFinding):
        # 🔍 BREAKPOINT 19: Resource pattern matching details
        logger.debug(f"🔍 Resource pattern matching:")
        ...
        if len(self.resource_pattern) > 0 and len(self.resource_type) > 0:
            # 🔍 BREAKPOINT 20: Both pattern and type matching
            ...
            if pattern_match and type_match:
                logger.debug(f"✅ Both pattern and type matched")
                return True
            else:
                logger.debug(f"❌ Pattern or type failed - Pattern: {pattern_match is not None}, Type: {type_match is not None}")
                return False
                
        elif len(self.resource_pattern) > 0:
            # 🔍 BREAKPOINT 21: Pattern-only matching
            pattern_match = re.search(self.resource_pattern, finding.resource_id)
            logger.debug(f"   Pattern-only match: {pattern_match is not None}")
            if pattern_match:
                logger.debug(f"✅ Pattern matched: '{pattern_match.group()}'")
                return True
            else:
                logger.debug(f"❌ Pattern failed to match")
                return False
                
        elif len(self.resource_type) > 0:
            # 🔍 BREAKPOINT 22: Type-only matching
            type_match = re.search(self.resource_type, finding.resource_type)
            logger.debug(f"   Type-only match: {type_match is not None}")
            if type_match:
                logger.debug(f"✅ Type matched")
                return True
            else:
                logger.debug(f"❌ Type failed to match")
                return False
        else:
            # 🔍 BREAKPOINT 23: No pattern or type specified
            logger.debug(f"❌ No pattern or type specified - rule invalid")
            return False
...
class SuppressionRuleEngine:
    def __init__(self):
        ...
        self.supported_regions_ssm_param_name = os.environ.get("SUPPORTED_REGIONS_SSM_PARAMETER",
                                                               "/security-tools/suppression-engine/supported-regions")
    ...
    def cache_eligible_for_suppression_finding(
            ...
            if match_result:
                ...
                if event_type == "historic" and old_status == "SUPPRESSED" and finding.udf and 'RuleId' in finding.udf:
                    if finding.udf['RuleId'] != rule.id:
                        ...
                        logger.info(
                            f"Caching historic finding {finding_id_arn} matched with rule {rule.id} which is eligible for re-suppression.")
                        return
                    else:
                        logger.info(
                            f"Historic finding {finding_id_arn} matched with rule {rule.id} but was already suppressed with this rule so skipping.")
                        return
                else:
                    self.cache_table.put_item(Item=item)
                    logger.info(
                        f"Caching finding {finding_id_arn} matched with rule {rule.id} which is eligible for suppression.")
                    return
            else:
        ...
        if event_type == "historic" and old_status == "SUPPRESSED":
            if finding.udf and 'RuleId' in finding.udf:
                ...
                self.cache_table.put_item(Item=item)
                logger.info(
                    f"Historic finding {finding_id_arn} and now has no rule matched so eligible for un-suppress.")
        else:
    ...
    def process_findings_from_cache_for_deleted_rules(self):
        """
            Process cached findings where the associated RuleId no longer exists.
            These findings should be unsuppressed (Workflow -> NEW) and an audit note appended.
        """
            ...
            try:
                ...
                for u in unprocessed_entries:
                    if "FindingIdentifier" in u and isinstance(u["FindingIdentifier"], dict):
                        ...
                        unprocessed_dict[u.get("FindingIdentifier", {}).get("Id",
                                                                            "unknown")] = f"Reason: {u.get('ErrorMessage', 'Unknown')}"
                    ...
                    with self.cache_table.batch_writer() as writer:
                        for ident_tuple in processed_ids:
                            ...
                            if cache_keys:
                                try:
                                    writer.delete_item(
                                        Key={"FindingId": cache_keys["FindingId"], "RuleId": cache_keys["RuleId"]}
                                    )
                                    logger.info(
                                        f"Deleted cache item for FindingId={cache_keys['FindingId']} RuleId={cache_keys['RuleId']}"
                                    )
                                except Exception as e:
                ...
                if processed_ids or unprocessed_dict:
                    ...
                    logger.info(
                        f"For deleted rules: processed_count={len(processed_ids)} unprocessed_count={len(unprocessed_dict)}")
    ...
    def process_findings_from_cache(self):
        for rule in self.rules:
            ...
            response = self.cache_table.scan(
                FilterExpression=Attr("RuleId").eq(rule.id),
            )
            items = response.get("Items", [])
            if not items:
            ...
            for i in range(0, len(items), 100):
                ...
                try:
                        ...
                        with self.cache_table.batch_writer() as writer:
                            for ident_tuple in processed_ids:
                                ...
                                if cache_keys:
                                    ...
                                    logger.warning(
                                        f"Processed identifier {ident_tuple} not found in chunk->cache mapping; skipping deletion.")
                    else:
                    ...
                    if processed_ids or unprocessed_dict:
                        ...
                        logger.info(
                            f"For RuleId={rule.id}: processed_count={len(processed_ids)} unprocessed_count={len(unprocessed_dict)}")
                        logger.info(f"Processed findings details: {processed_ids}")
                        if unprocessed_dict:
                            logger.error(f"Unprocessed findings details: {unprocessed_dict}")
    ...
    '''
        This is scheduled trigger which will re-evaluate all historic findings and will
        1. Reopen the findings if not eligible anymore either bcos :
            1.1. rule expired 
            1.2. rule was changed or deleted and is not matching anymore
        2. Suppress any historic finding which is in NEW state and now a rule exists to suppress it.
    '''
            ...
            for page_num, page in enumerate(
                ...
                for finding in findings:
                    ...
                    if not rule_id:
                        ...
                        logger.info(
                            f"Suppressed finding {sec_hub_finding.id} has invalid RuleId={rule_id} so reevaluating")
                    else:
...
def lambda_handler(event, context):
    # 🔍 BREAKPOINT 1: Lambda entry point
    logger.info(f"🚀 Lambda handler started")
    logger.info(f"Incoming event : {repr(event)}")
    
    # 🔍 BREAKPOINT 2: SuppressionRuleEngine initialization
    logger.info("🔧 Initializing SuppressionRuleEngine...")
    suppression_engine = SuppressionRuleEngine()
    logger.info(f"✅ Loaded {len(suppression_engine.rules)} rules")

    # 🔍 BREAKPOINT 3: Event type determination
    scheduler_type = event.get("scheduler-type")
    event_source = event.get("source")
    logger.info(f"📋 Event analysis: source={event_source}, scheduler_type={scheduler_type}")

    # --- Case 2: New Security Hub finding ---
    if event_source == "aws.securityhub":
        # 🔍 BREAKPOINT 4: Security Hub event processing
        logger.info("🎯 Event type : SecurityHub")
        findings = event.get("detail", {}).get("findings", [])
        logger.info(f"📊 Processing {len(findings)} findings")
        
        for i, finding in enumerate(findings, 1):
            # 🔍 BREAKPOINT 5: Individual finding processing
            logger.info(f"🔍 Processing finding {i}/{len(findings)}")
            sec_hub_finding = SecurityHubFinding(finding)
            logger.info(f"✅ Transformed to SecurityHubFinding: {sec_hub_finding}")
            
            # 🔍 BREAKPOINT 6: Cache eligibility check
            logger.info(f"🎯 Checking eligibility for suppression...")
